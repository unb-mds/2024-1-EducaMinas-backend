...
import { genericData, disorderedData, incompleteData, perfectData } from '../database/memory/data/indicadoresMemoryData'; 
...
describe('Indicadores Service', () => {
  ...
  test('should return an Error when not exists register with indicador filter', async () => {
    ...
    await expect(indicadoresService.execute(incorrectIndicadorInput)).rejects.toThrow('Nenhum dado foi encontrado para estes filtros.');
  });
